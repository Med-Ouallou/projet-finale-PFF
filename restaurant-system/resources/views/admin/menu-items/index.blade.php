        ...
        <div @click.outside="showEditModal = false" class="w-full sm:max-w-xl bg-white border border-gray-100 shadow-2xl rounded-3xl overflow-hidden relative">
            <div class="flex justify-between items-center py-5 px-6 border-b border-gray-100">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-bold font-heading text-gray-900 leading-none">Modifier le plat</h3>
                        <p class="text-xs text-gray-400 mt-0.5">Mettez à jour les informations du plat.</p>
                    </div>
                </div>
            ...
            <form method="POST" :action="`/admin/menu-items/${editForm.id}`" enctype="multipart/form-data" class="p-6 space-y-5 overflow-y-auto max-h-[70vh]">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-800 mb-1.5">Catégorie <span class="text-red-400">*</span></label>
                        <x-ui.select name="category_id" x-model="editForm.category_id" required class="py-3 px-4 block w-full border border-gray-200 rounded-xl text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 bg-gray-50 transition">
                            <option value="">Sélectionner...</option>
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="price" step="0.01" min="0" required x-model="editForm.price"
                                class="py-3 ps-9 px-4 block w-full border border-gray-200 rounded-xl text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 bg-gray-50 transition"
                                placeholder="0.00">
                        </div>
                    </div>
                    ...
                    <div class="col-span-2">
                        <label class="block text-sm font-bold text-gray-800 mb-1.5">Description</label>
                        <textarea name="description" x-model="editForm.description" class="py-3 px-4 block w-full border border-gray-200 rounded-xl text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 bg-gray-50 transition" rows="3" placeholder="Ingrédients, allergènes, remarques..."></textarea>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between p-4 bg-emerald-50/60 rounded-2xl border border-emerald-100">
                    ...
                    <x-ui.select name="status" x-model="editForm.status" class="py-2 px-3 text-sm rounded-xl border border-gray-200 bg-white">
                        <option value="available">Disponible</option>
                        <option value="unavailable">Indisponible</option>
                    </x-ui.select>
                </div>
